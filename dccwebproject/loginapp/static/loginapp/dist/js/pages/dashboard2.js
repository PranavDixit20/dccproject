...
$(function ()    {
  ...
  var salesChartOptions = {
    ...
    // Boolean - whether to maintain
    //the starting aspect ratio or not
  ...
  var pieOptions     = {
    ...
    animationEasing: 'easeOutBounce',    // Boolean -  animate the rotation of the Doughnut
    ...
    responsive: true,  // Boolean - if set to false, will take up entire container
    maintainAspectRatio: false,   // String - A legend template
    legendTemplate: '<ul class=\'<%=name.toLowerCase()%>-legend\'><% for (var i=0; i<segments.length; i++){%><li><span style=\'background-color:<%=segments[i].fillColor%>\'></span><%if(segments[i].label){%><%=segments[i].label%><%}%></li><%}%></ul>',
    tooltipTemplate: '<%=value %> <%=label%> users',
  };// Create pie or douhnut chart
  // You can switch between pie and douhnut using the method below.
  pieChart.Doughnut(PieData, pieOptions);  // - END PIE CHART -
